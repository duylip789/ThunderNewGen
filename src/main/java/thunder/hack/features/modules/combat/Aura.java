...
public class Aura extends Module {
    ...
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.ThunderHackV2) || esp.is(ESP.NewGen));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.ThunderHackV2) || esp.is(ESP.NewGen)).addToGroup(espGroup);
    ...
    private static class Trail {
        ...
        public Trail(float offset) { this.offset = offset; }
        public void update(Entity target) {
            long time = System.currentTimeMillis();
            double sin = Math.sin((time + offset * 1000) / 400.0) * target.getWidth() * 1.1;
            double cos = Math.cos((time + offset * 1000) / 400.0) * target.getWidth() * 1.1;
            double y = target.getY() + (target.getHeight() / 2.0) + Math.sin((time + offset * 1500) / 600.0) * (target.getHeight() / 2.0);
            points.add(0, new Vec3d(target.getX() + sin, y, target.getZ() + cos));
            int max = ModuleManager.aura.espLength.getValue();
            while (points.size() > max) points.remove(points.size() - 1);
        }
    }

    public Aura() {
        super("Aura", Category.COMBAT);
        for (int i = 0; i < 3; i++) trails.add(new Trail(i * (2f / 3f)));
    }
    ...
    public void auraLogic() {
        ...
        if (esp.is(ESP.NewGen)) { for (Trail t : trails) t.update(target); }
    ...
    public void attack() {
        ...
        int prev = switchMethod(); mc.interactionManager.attackEntity(mc.player, target);
    ...
    private int switchMethod() { int p = -1; SearchInvResult r = InventoryUtility.getSwordHotBar(); if (r.found() && switchMode.getValue() != Switch.None) { if (switchMode.getValue() == Switch.Silent) p = mc.player.getInventory().selectedSlot; r.switchTo(); } return p; }
    ...
    @EventHandler public void onUpdate(PlayerUpdateEvent e) { if (!pauseTimer.passedMs(1000) || (mc.player.isUsingItem() && pauseWhileEating.getValue())) return; if(pauseBaritone.getValue() && ThunderHack.baritone){ boolean isT = (target != null); if (isT && !wasTargeted) { BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("pause"); wasTargeted = true; } else if (!isT && wasTargeted) { BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("resume"); wasTargeted = false; } } resolvePlayers(); auraLogic(); restorePlayers(); hitTicks--; }
    @EventHandler public void onSync(EventSync e) { if (!pauseTimer.passedMs(1000) || (mc.player.isUsingItem() && pauseWhileEating.getValue()) || !haveWeapon()) return; if (target != null && rotationMode.getValue() != Mode.None && rotationMode.getValue() != Mode.Grim) { mc.player.setYaw(rotationYaw); mc.player.setPitch(rotationPitch); } else { rotationYaw = mc.player.getYaw(); rotationPitch = mc.player.getPitch(); } if (oldDelay.getValue().isEnabled() && minCPS.getValue() > maxCPS.getValue()) minCPS.setValue(maxCPS.getValue()); if (target != null && pullDown.getValue() && (mc.player.hasStatusEffect(StatusEffects.JUMP_BOOST) || !onlyJumpBoost.getValue())) mc.player.addVelocity(0f, -pullValue.getValue() / 1000f, 0f); }
    @EventHandler public void onPacketSend(PacketEvent.@NotNull Send e) { if (e.getPacket() instanceof PlayerInteractEntityC2SPacket pie && Criticals.getInteractType(pie) != Criticals.InteractType.ATTACK && target != null) e.cancel(); }
    @EventHandler public void onPacketReceive(PacketEvent.@NotNull Receive e) { if (e.getPacket() instanceof EntityStatusS2CPacket s && s.getStatus() == 30 && s.getEntity(mc.world) != null && target != null && s.getEntity(mc.world) == target) Managers.NOTIFICATION.publicity("Aura", isRu() ? ("Успешно сломали щит игроку " + target.getName().getString()) : ("Succesfully destroyed " + target.getName().getString() + "'s shield"), 2, Notification.Type.SUCCESS); if (e.getPacket() instanceof PlayerPositionLookS2CPacket && tpDisable.getValue()) disable(isRu() ? "Отключаю из-за телепортации!" : "Disabling due to tp!"); if (e.getPacket() instanceof EntityStatusS2CPacket pac && pac.getStatus() == 3 && pac.getEntity(mc.world) == mc.player && deathDisable.getValue()) disable(isRu() ? "Отключаю из-за смерти!" : "Disabling due to death!"); }

    @Override public void onEnable() { target = null; lookingAtHitbox = false; rotationPoint = Vec3d.ZERO; rotationMotion = Vec3d.ZERO; rotationYaw = mc.player.getYaw(); rotationPitch = mc.player.getPitch(); delayTimer.reset(); }

    private boolean autoCrit() {
        boolean r = !smartCrit.getValue().isEnabled() || mc.player.getAbilities().flying || (mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) || mc.player.hasStatusEffect(StatusEffects.BLINDNESS) || mc.player.hasStatusEffect(StatusEffects.SLOW_FALLING) || Managers.PLAYER.isInWeb();
        ...
        if (!mc.options.jumpKey.isPressed() && (!ModuleManager.targetStrafe.isEnabled() || !ModuleManager.targetStrafe.jump.getValue()) && (!ModuleManager.speed.isEnabled() || mc.player.isOnGround()) && !onlySpace.getValue() && !autoJump.getValue()) return true;
        ...
        return r || (!mc.player.isOnGround() && mc.player.fallDistance > (shouldRandomizeFallDistance() ? MathUtility.random(0.15f, 0.7f) : critFallDistance.getValue()));
    }

    private boolean shieldBreaker(boolean instant) {
        int axe = InventoryUtility.getAxe().slot(); if (axe == -1 || !shieldBreaker.getValue() || !(target instanceof PlayerEntity)) return false;
        PlayerEntity t = (PlayerEntity) target; if (!t.isUsingItem() && !instant) return false;
        if (t.getOffHandStack().getItem() != Items.SHIELD && t.getMainHandStack().getItem() != Items.SHIELD) return false;
        if (axe >= 9) { mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, axe, mc.player.getInventory().selectedSlot, SlotActionType.SWAP, mc.player); sendPacket(new CloseHandledScreenC2SPacket(mc.player.currentScreenHandler.syncId)); mc.interactionManager.attackEntity(mc.player, target); swingHand(); mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, axe, mc.player.getInventory().selectedSlot, SlotActionType.SWAP, mc.player); sendPacket(new CloseHandledScreenC2SPacket(mc.player.currentScreenHandler.syncId)); }
        else { sendPacket(new UpdateSelectedSlotC2SPacket(axe)); mc.interactionManager.attackEntity(mc.player, target); swingHand(); sendPacket(new UpdateSelectedSlotC2SPacket(mc.player.getInventory().selectedSlot)); }
        hitTicks = 10; return true;
    }

    private void swingHand() { if (attackHand.getValue() == AttackHand.OffHand) mc.player.swingHand(Hand.OFF_HAND); else if (attackHand.getValue() == AttackHand.MainHand) mc.player.swingHand(Hand.MAIN_HAND); }
    ...
    private void updateTarget() { Entity c = findTarget(); if (target == null) { target = c; return; } if (sort.getValue() == Sort.FOV || !lockTarget.getValue()) target = c; if (c instanceof ProjectileEntity) target = c; if (skipEntity(target)) target = null; }
    ...
    private void calcRotations(boolean ready) {
        ...
        if (target == null) return;
        Vec3d tv = (mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) ? target.getEyePos() : getLegitLook(target);
        if (tv == null) return;
        pitchAcceleration = Managers.PLAYER.checkRtx(rotationYaw, rotationPitch, getRange() + aimRange.getValue(), getRange() + aimRange.getValue(), rayTrace.getValue()) ? aimedPitchStep.getValue() : Math.min(pitchAcceleration * pitchAccelerate.getValue(), maxPitchStep.getValue());
        ...
        float ys = rotationMode.getValue() != Mode.Track ? 360f : random(minYawStep.getValue(), maxYawStep.getValue());
        float ps = rotationMode.getValue() != Mode.Track ? 180f : Managers.PLAYER.ticksElytraFlying > 5 ? 180 : (pitchAcceleration + random(-1f, 1f));
        if (ready) switch (accelerateOnHit.getValue()) { case Yaw -> ys = 180f; case Pitch -> ps = 90f; case Both -> { ys = 180f; ps = 90f; } }
        if (dy > 180) dy -= 180;
        float day = MathHelper.clamp(MathHelper.abs(dy), -ys, ys), dap = MathHelper.clamp(dp, -ps, ps);
        float ny = rotationYaw + (dy > 0 ? day : -day), np = MathHelper.clamp(rotationPitch + dap, -90.0F, 90.0F);
        double gcd = (Math.pow(mc.options.getMouseSensitivity().getValue() * 0.6 + 0.2, 3.0)) * 1.2;
        if (trackticks > 0 || rotationMode.getValue() == Mode.Track) { rotationYaw = (float) (ny - (ny - rotationYaw) % gcd); rotationPitch = (float) (np - (np - rotationPitch) % gcd); }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case NewGen -> {
                Render3DEngine.drawSphere(stack, 0.1f, HudEditor.getColor(0).getRGB(), 10, 10);
                for (Trail t : trails) {
                    if (t.points.size() < 2) continue;
                    for (int i = 0; i < t.points.size() - 1; i++) {
                        Color base = HudEditor.getColor(i * 5);
                        int alpha = (int) (255 * (1.0f - (float) i / t.points.size()));
                        Render3DEngine.drawLine(t.points.get(i), t.points.get(i+1), new Color(base.getRed(), base.getGreen(), base.getBlue(), alpha));
                    }
                }
            }
        }
        if (clientLook.getValue() && rotationMode.getValue() != Mode.None) { mc.player.setYaw((float) Render2DEngine.interpolate(mc.player.prevYaw, rotationYaw, Render3DEngine.getTickDelta())); mc.player.setPitch((float) Render2DEngine.interpolate(mc.player.prevPitch, rotationPitch, Render3DEngine.getTickDelta())); }
    }

    @Override public void onDisable() { target = null; }
    public float getSquaredRotateDistance() { float d = getRange() + aimRange.getValue(); if ((mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) && target != null) d += 4f; if (ModuleManager.strafe.isEnabled()) d += 4f; return (rotationMode.getValue() != Mode.Track || rayTrace.getValue() == RayTrace.OFF) ? getRange() * getRange() : d * d; }

    public Vec3d getLegitLook(Entity target) {
        float minXZ = 0.003f, maxXZ = 0.03f, minY = 0.001f, maxY = 0.03f;
        ...
        if (rotationPoint.x >= (lx - 0.05) / 2f) rotationMotion = new Vec3d(-random(minXZ, maxXZ), rotationMotion.getY(), rotationMotion.getZ());
        if (rotationPoint.y >= ly) rotationMotion = new Vec3d(rotationMotion.getX(), -random(minY, maxY), rotationMotion.getZ());
        if (rotationPoint.z >= (lz - 0.05) / 2f) rotationMotion = new Vec3d(rotationMotion.getX(), rotationMotion.getY(), -random(minXZ, maxXZ));
        if (rotationPoint.x <= -(lx - 0.05) / 2f) rotationMotion = new Vec3d(random(minXZ, 0.03f), rotationMotion.getY(), rotationMotion.getZ());
        if (rotationPoint.y <= 0.05) rotationMotion = new Vec3d(rotationMotion.getX(), random(minY, maxY), rotationMotion.getZ());
        if (rotationPoint.z <= -(lz - 0.05) / 2f) rotationMotion = new Vec3d(rotationMotion.getX(), rotationMotion.getY(), random(minXZ, maxXZ));
        rotationPoint.add(random(-0.03f, 0.03f), 0f, random(-0.03f, 0.03f));
        if (!mc.player.canSee(target) && Objects.requireNonNull(wallsBypass.getValue()) == WallsBypass.V1) return target.getPos().add(random(-0.15, 0.15), ly, random(-0.15, 0.15));
        if (!Managers.PLAYER.checkRtx(rotationYaw, rotationPitch, getRange(), getWallRange(), rayTrace.getValue())) {
            ...
            if (PlayerUtility.squaredDistanceFromEyes(target.getPos().add(0, target.getEyeHeight(target.getPose()) / 2f, 0)) <= attackRange.getPow2Value() && Managers.PLAYER.checkRtx(r1[0], r1[1], getRange(), 0, rayTrace.getValue())) rotationPoint = new Vec3d(random(-0.1f, 0.1f), target.getEyeHeight(target.getPose()) / (random(1.8f, 2.5f)), random(-0.1f, 0.1f));
            else { float hb = (float) (lx / 2f); for (float x1 = -hb; x1 <= hb; x1 += 0.05f) for (float z1 = -hb; z1 <= hb; z1 += 0.05f) for (float y1 = 0.05f; y1 <= ly; y1 += 0.15f) { Vec3d v1 = new Vec3d(target.getX() + x1, target.getY() + y1, target.getZ() + z1); if (PlayerUtility.squaredDistanceFromEyes(v1) <= attackRange.getPow2Value()) { float[] rot = Managers.PLAYER.calcAngle(v1); if (Managers.PLAYER.checkRtx(rot[0], rot[1], getRange(), 0, rayTrace.getValue())) { rotationPoint = new Vec3d(x1, y1, z1); return target.getPos().add(rotationPoint); } } } }
        }
    ...
    public boolean isInRange(Entity target) {
        ...
        for (float x1 = -hb; x1 <= hb; x1 += 0.15f) for (float z1 = -hb; z1 <= hb; z1 += 0.15f) for (float y1 = 0.05f; y1 <= target.getBoundingBox().getLengthY(); y1 += 0.25f) { Vec3d v = new Vec3d(target.getX() + x1, target.getY() + y1, target.getZ() + z1); if (PlayerUtility.squaredDistanceFromEyes(v) <= getSquaredRotateDistance()) { float[] r = Managers.PLAYER.calcAngle(v); if (Managers.PLAYER.checkRtx(r[0], r[1], (float) Math.sqrt(getSquaredRotateDistance()), getWallRange(), rayTrace.getValue())) return true; } }
        return false;
    }

    public Entity findTarget() {
        List<LivingEntity> fs = new CopyOnWriteArrayList<>();
        for (Entity e : mc.world.getEntities()) { if ((e instanceof ShulkerBulletEntity || e instanceof FireballEntity) && e.isAlive() && isInRange(e) && Projectiles.getValue()) return e; if (!skipEntity(e) && e instanceof LivingEntity) fs.add((LivingEntity) e); }
        return switch (sort.getValue()) {
            case LowestDistance -> fs.stream().min(Comparator.comparing(e -> (mc.player.squaredDistanceTo(e.getPos())))).orElse(null);
            case HighestDistance -> fs.stream().max(Comparator.comparing(e -> (mc.player.squaredDistanceTo(e.getPos())))).orElse(null);
            case FOV -> fs.stream().min(Comparator.comparing(this::getFOVAngle)).orElse(null);
            case LowestHealth -> fs.stream().min(Comparator.comparing(e -> (e.getHealth() + e.getAbsorptionAmount()))).orElse(null);
            case HighestHealth -> fs.stream().max(Comparator.comparing(e -> (e.getHealth() + e.getAbsorptionAmount()))).orElse(null);
            case LowestDurability -> fs.stream().min(Comparator.comparing(e -> { float v = 0; for (ItemStack a : e.getArmorItems()) if (a != null && !a.getItem().equals(Items.AIR)) v += ((a.getMaxDamage() - a.getDamage()) / (float) a.getMaxDamage()); return v; })).orElse(null);
            case HighestDurability -> fs.stream().max(Comparator.comparing(e -> { float v = 0; for (ItemStack a : e.getArmorItems()) if (a != null && !a.getItem().equals(Items.AIR)) v += ((a.getMaxDamage() - a.getDamage()) / (float) a.getMaxDamage()); return v; })).orElse(null);
        };
    }
    ...
    private boolean skipEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity p) { if (ModuleManager.antiBot.isEnabled() && AntiBot.bots.contains(entity)) return true; if (p == mc.player || Managers.FRIEND.isFriend(p)) return true; if (p.isCreative() && ignoreCreative.getValue()) return true; if (p.getArmor() == 0 && ignoreNaked.getValue()) return true; if (p.isInvisible() && ignoreInvisible.getValue()) return true; if (p.getTeamColorValue() == mc.player.getTeamColorValue() && ignoreTeam.getValue() && mc.player.getTeamColorValue() != 16777215) return true; }
    ...
    private boolean skipNotSelected(Entity e) { if (e instanceof SlimeEntity && !Slimes.getValue()) return true; if (e instanceof HostileEntity he && (!hostiles.getValue() || (onlyAngry.getValue() && !he.isAngryAt(mc.player)))) return true; if (e instanceof PlayerEntity && !Players.getValue()) return true; if (e instanceof VillagerEntity && !Villagers.getValue()) return true; if (e instanceof MobEntity && !Mobs.getValue()) return true; return e instanceof AnimalEntity && !Animals.getValue(); }
    private float getFOVAngle(@NotNull LivingEntity e) { double dx = e.getX() - mc.player.getX(), dz = e.getZ() - mc.player.getZ(); float y = (float) MathHelper.wrapDegrees(Math.toDegrees(Math.atan2(dz, dx)) - 90.0); return Math.abs(y - MathHelper.wrapDegrees(mc.player.getYaw())); }
    ...
    public static class Position {
        private double x, y, z; private int t;
        public Position(double x, double y, double z) { this.x = x; this.y = y; this.z = z; }
        public boolean shouldRemove() { return t++ > ModuleManager.aura.backTicks.getValue(); }
