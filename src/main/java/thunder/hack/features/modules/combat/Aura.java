...
public class Aura extends Module {
    ...
    private boolean skipRayTraceCheck() {
        return rotationMode.getValue() == Mode.None || rayTrace.getValue() == RayTrace.OFF
                ...
                || (mc.world != null && mc.world.getBlockCollisions(mc.player, mc.player.getBoundingBox().expand(-0.25, 0.0, -0.25).offset(0.0, 1, 0.0)).iterator().hasNext())));
    }
    ...
    private boolean @NotNull [] preAttack() {
        ...
        if (sprintActive && (sprint.is(SprintMode.HvH) || sprint.is(SprintMode.SMP)))
            disableSprint();
    ...
    public void resolvePlayers() {
        if (resolver.not(Resolver.Off) && mc.world != null)
    ...
    public void handleKill() {
        if (target instanceof LivingEntity && (((LivingEntity) target).getHealth() <= 0 || !((LivingEntity) target).isAlive()))
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        if (!pauseTimer.passedMs(1000) || mc.player == null)
            return;
        ...
        if (ghostList.size() > espLength.getValue()) {
            ghostList.remove(0);
        }
    }
        ...
        switch (esp.getValue()) {
            ...
            case GhostV2 -> {
                for (GhostData gd : ghostList) {
                    ...
                    stack.translate(gd.x - camPos.x, gd.y - camPos.y, gd.z - camPos.z);
    ...
    public Vec3d getLegitLook(Entity target) {
        ...
        if (rotationPoint.y <= 0.05)
            rotationMotion = new Vec3d(rotationMotion.getX(), random(minMotionY, maxMotionY), rotationPoint.z);
        if (rotationPoint.z <= -(lenghtZ - 0.05) / 2f)
            rotationMotion = new Vec3d(rotationMotion.getX(), rotationMotion.getY(), random(minMotionXZ, maxMotionXZ));
                ...
                for (float x1 = -halfBox; x1 <= halfBox; x1 += 0.05f) {
                    for (float z1 = -halfBox; z1 <= halfBox; z1 += 0.05f) {
                        for (float y1 = 0.05f; y1 <= target.getBoundingBox().getLengthY(); y1 += 0.15f) {
                            ...
                            if (Managers.PLAYER.checkRtx(rot[0], rot[1], getRange(), 0, rayTrace.getValue())) {
    ...
    public static class Position {
        ...
        public boolean shouldRemove(Position p) {
            return p.ticks++ > ModuleManager.aura.backTicks.getValue();
        }
    }
