...
public class Aura extends Module {
    // Class Position phải là PUBLIC để MixinEntityLiving có thể truy cập
    ...
    // Elytra Target Advanced
    public final Setting<BooleanSettingGroup> elytraTargetGroup = new Setting<>("ElytraTarget", new BooleanSettingGroup(false));
    public final Setting<Bind> elytraTargetBind = new Setting<>("ETarget Bind", new Bind(-1, false, false)).addToGroup(elytraTargetGroup);
    public final Setting<Boolean> autoFirework = new Setting<>("AutoFirework", true).addToGroup(elytraTargetGroup);
    public final Setting<Float> heightGain = new Setting<>("HeightGain", 1.5f, 0f, 4.0f).addToGroup(elytraTargetGroup);
    ...
    private void handleSprintMode() {
        ...
        switch (sprintMode.getValue()) {
            case HvH -> {
                if (mc.player.fallDistance > 0.05) mc.player.setSprinting(false);
                else if (mc.player.forwardSpeed > 0) mc.player.setSprinting(true);
            }
            case Legit -> {
                if (mc.player.forwardSpeed <= 0 || mc.player.isHorizontalCollision || mc.player.isUsingItem()) mc.player.setSprinting(false);
            }
            case SMP -> mc.player.setSprinting(mc.player.forwardSpeed > 0 && !mc.player.isUsingItem());
            case None -> {}
        }
    }

    private void handleElytraTarget() {
        if (!elytraTargetGroup.getValue().isEnabled() || target == null || !mc.player.isFallFlying()) return;
        double desiredY = target.getY() + heightGain.getValue();
        if (mc.player.getY() < desiredY) {
            mc.player.addVelocity(0, 0.15, 0);
            if (autoFirework.getValue() && fireworkTimer.passedMs(700)) {
                int fireworkSlot = InventoryUtility.getFirework().slot();
                if (fireworkSlot != -1) {
                    int prevSlot = mc.player.getInventory().selectedSlot;
                    InventoryUtility.switchTo(fireworkSlot);
    ...
    // Các hàm phụ trợ và logic gốc được giữ nguyên từ file người dùng cung cấp
    ...
    private int switchMethod() {
        int prev = -1; SearchInvResult r = InventoryUtility.getSwordHotBar();
        if (r.found() && switchMode.getValue() != Switch.None) { if (switchMode.getValue() == Switch.Silent) prev = mc.player.getInventory().selectedSlot; r.switchTo(); }
        return prev;
    }
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if(pauseBaritone.getValue() && ThunderHack.baritone){
            boolean isT = (target != null);
            if (isT && !wasTargeted) { BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("pause"); wasTargeted = true; }
            else if (!isT && wasTargeted) { BaritoneAPI.getProvider().getPrimaryBaritone().getCommandManager().execute("resume"); wasTargeted = false; }
        }
    ...
    @EventHandler public void onPacketReceive(PacketEvent.@NotNull Receive e) {
        if (e.getPacket() instanceof EntityStatusS2CPacket s && s.getStatus() == 30 && s.getEntity(mc.world) != null && target != null && s.getEntity(mc.world) == target) Managers.NOTIFICATION.publicity("Aura", isRu() ? ("Успешно сломали щит игроку " + target.getName().getString()) : ("Succesfully destroyed " + target.getName().getString() + "'s shield"), 2, Notification.Type.SUCCESS);
    ...
    private boolean autoCrit() {
        boolean skip = !smartCrit.getValue().isEnabled() || mc.player.getAbilities().flying || (mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) || mc.player.hasStatusEffect(StatusEffects.BLINDNESS) || mc.player.hasStatusEffect(StatusEffects.SLOW_FALLING) || Managers.PLAYER.isInWeb();
        ...
        return skip || (!mc.player.isOnGround() && mc.player.fallDistance > (shouldRandomizeFallDistance() ? MathUtility.random(0.15f, 0.7f) : critFallDistance.getValue()));
    }
    ...
    private boolean shieldBreaker(boolean instant) {
        ...
        if (axe >= 9) {
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, axe, mc.player.getInventory().selectedSlot, SlotActionType.SWAP, mc.player);
            sendPacket(new CloseHandledScreenC2SPacket(mc.player.currentScreenHandler.syncId)); mc.interactionManager.attackEntity(mc.player, target); swingHand();
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, axe, mc.player.getInventory().selectedSlot, SlotActionType.SWAP, mc.player);
    ...
    private void calcRotations(boolean ready) {
        ...
        if (target == null) return;
        Vec3d v = (mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) ? target.getEyePos() : getLegitLook(target);
        if (v == null) return;
        ...
        float dy = wrapDegrees((float) wrapDegrees(Math.toDegrees(Math.atan2(v.z - mc.player.getZ(), (v.x - mc.player.getX()))) - 90) - rotationYaw) + (wallsBypass.is(WallsBypass.V2) && !ready && !mc.player.canSee(target) ? 20 : 0);
        float dp = ((float) (-Math.toDegrees(Math.atan2(v.y - (mc.player.getPos().y + mc.player.getEyeHeight(mc.player.getPose())), Math.sqrt(Math.pow((v.x - mc.player.getX()), 2) + Math.pow(v.z - mc.player.getZ(), 2))))) - rotationPitch);
    ...
    private boolean skipEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity p) { if (ModuleManager.antiBot.isEnabled() && AntiBot.bots.contains(entity)) return true; if (p == mc.player || Managers.FRIEND.isFriend(p)) return true; if (p.isCreative() && ignoreCreative.getValue()) return true; if (p.getArmor() == 0 && ignoreNaked.getValue()) return true; if (p.getTeamColorValue() == mc.player.getTeamColorValue() && ignoreTeam.getValue() && mc.player.getTeamColorValue() != 16777215) return true; }
