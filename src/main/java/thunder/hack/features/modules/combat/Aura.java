...
public class Aura extends Module {
    ...
    public final Setting<Boolean> onlyWeapon = new Setting<>("OnlyWeapon", false, v -> switchMode.getValue() != Switch.None);
    ...
    public void onPacketReceive(PacketEvent.@NotNull Receive e) {
        if (e.getPacket() instanceof EntityStatusS2CPacket status && status.getStatus() == 30 && status.getEntity(mc.world) != null && target != null && status.getEntity(mc.world) == target)
    ...
    public void onEnable() {
        target = null; lookingAtHitbox = false; rotationPoint = Vec3d.ZERO; rotationMotion = Vec3d.ZERO;
        rotationYaw = mc.player.getYaw(); rotationPitch = mc.player.getPitch(); delayTimer.reset(); trailParticles.clear();
    }

    private boolean autoCrit() {
        boolean skipCrit = !smartCrit.getValue().isEnabled() || mc.player.getAbilities().flying || (mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) || mc.player.hasStatusEffect(StatusEffects.BLINDNESS) || mc.player.hasStatusEffect(StatusEffects.SLOW_FALLING) || Managers.PLAYER.isInWeb();
        ...
        boolean mergeTS = !ModuleManager.targetStrafe.isEnabled() || !ModuleManager.targetStrafe.jump.getValue();
        boolean mergeSpd = !ModuleManager.speed.isEnabled() || mc.player.isOnGround();
        if (!mc.options.jumpKey.isPressed() && mergeTS && mergeSpd && !onlySpace.getValue() && !autoJump.getValue()) return true;
        ...
        return skipCrit || (!mc.player.isOnGround() && mc.player.fallDistance > (shouldRandomizeFallDistance() ? MathUtility.random(0.15f, 0.7f) : critFallDistance.getValue()));
    }
    ...
    private boolean shieldBreaker(boolean instant) { 
        ...
        if (axeSlot >= 9) {
            ...
            mc.interactionManager.attackEntity(mc.player, target); swingHand();
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, axeSlot, mc.player.getInventory().selectedSlot, SlotActionType.SWAP, mc.player);
            sendPacket(new CloseHandledScreenC2SPacket(mc.player.currentScreenHandler.syncId));
        } else {
    ...
    private void updateTarget() {
        ...
        if (target == null || sort.getValue() == Sort.FOV || !lockTarget.getValue() || candidat instanceof ProjectileEntity) target = candidat;
    ...
    private void calcRotations(boolean ready) {
        ...
        if (ready) { if (accelerateOnHit.getValue() == AccelerateOnHit.Yaw || accelerateOnHit.getValue() == AccelerateOnHit.Both) yawStep = 180f; if (accelerateOnHit.getValue() == AccelerateOnHit.Pitch || accelerateOnHit.getValue() == AccelerateOnHit.Both) pitchStep = 90f; }
        ...
        if (trackticks > 0 || rotationMode.getValue() == Mode.Track) { rotationYaw = (float) (newYaw - (newYaw - rotationYaw) % gcd); rotationPitch = (float) (newPitch - (newPitch - rotationPitch) % gcd); }
        else { rotationYaw = mc.player.getYaw(); rotationPitch = mc.player.getPitch(); }
    ...
    public float getSquaredRotateDistance() { float dst = (rotationMode.getValue() != Mode.Track || rayTrace.getValue() == RayTrace.OFF) ? getRange() : getRange() + aimRange.getValue(); if ((mc.player.isFallFlying() || ModuleManager.elytraPlus.isEnabled()) && target != null) dst += 4f; if (ModuleManager.strafe.isEnabled()) dst += 4f; return dst * dst; }
    ...
    public Vec3d getLegitLook(Entity target) {
        ...
        if (rotationPoint.x >= (lx - 0.05) / 2f) rotationMotion = new Vec3d(-random(0.003f, 0.03f), rotationMotion.getY(), rotationMotion.getZ());
        if (rotationPoint.y >= ly) rotationMotion = new Vec3d(rotationMotion.getX(), -random(0.001f, 0.03f), rotationMotion.getZ());
        if (rotationPoint.z >= (lz - 0.05) / 2f) rotationMotion = new Vec3d(rotationMotion.getX(), rotationMotion.getY(), -random(0.003f, 0.03f));
        if (rotationPoint.x <= -(lx - 0.05) / 2f) rotationMotion = new Vec3d(random(0.003f, 0.03f), rotationMotion.getY(), rotationMotion.getZ());
        ...
        if (!Managers.PLAYER.checkRtx(rotationYaw, rotationPitch, getRange(), getWallRange(), rayTrace.getValue())) {
            ...
            else { float hb = (float) (lx / 2f); for (float x1 = -hb; x1 <= hb; x1 += 0.05f) for (float z1 = -hb; z1 <= hb; z1 += 0.05f) for (float y1 = 0.05f; y1 <= ly; y1 += 0.15f) { Vec3d v1 = new Vec3d(target.getX() + x1, target.getY() + y1, target.getZ() + z1); if (PlayerUtility.squaredDistanceFromEyes(v1) <= attackRange.getPow2Value()) { float[] r = Managers.PLAYER.calcAngle(v1); if (Managers.PLAYER.checkRtx(r[0], r[1], getRange(), 0, rayTrace.getValue())) { rotationPoint = new Vec3d(x1, y1, z1); break; } } } }
        }
    ...
    public Entity findTarget() {
        ...
        for (Entity ent : mc.world.getEntities()) { if ((ent instanceof ShulkerBulletEntity || ent instanceof FireballEntity) && ent.isAlive() && isInRange(ent) && Projectiles.getValue()) return ent; if (!skipEntity(ent) && ent instanceof LivingEntity) stage.add((LivingEntity) ent); }
        return switch (sort.getValue()) {
            ...
            case LowestDurability -> stage.stream().min(Comparator.comparing(e -> { float v = 0; for (ItemStack i : e.getArmorItems()) if (i != null && !i.getItem().equals(Items.AIR)) v += ((i.getMaxDamage() - i.getDamage()) / (float) i.getMaxDamage()); return v; })).orElse(null);
            case HighestDurability -> stage.stream().max(Comparator.comparing(e -> { float v = 0; for (ItemStack i : e.getArmorItems()) if (i != null && !i.getItem().equals(Items.AIR)) v += ((i.getMaxDamage() - i.getDamage()) / (float) i.getMaxDamage()); return v; })).orElse(null);
        };
    }
    ...
    private boolean isBullet(Entity e) { return (e instanceof ShulkerBulletEntity || e instanceof FireballEntity) && e.isAlive() && PlayerUtility.squaredDistanceFromEyes(e.getPos()) < getSquaredRotateDistance() && Projectiles.getValue(); }
    private boolean skipNotSelected(Entity e) { if (e instanceof SlimeEntity && !Slimes.getValue()) return true; if (e instanceof HostileEntity he) return !hostiles.getValue() || (onlyAngry.getValue() && !he.isAngryAt(mc.player)); if (e instanceof PlayerEntity && !Players.getValue()) return true; if (e instanceof VillagerEntity && !Villagers.getValue()) return true; if (e instanceof MobEntity && !Mobs.getValue()) return true; return e instanceof AnimalEntity && !Animals.getValue(); }
    private float getFOVAngle(@NotNull LivingEntity e) { float yaw = (float) MathHelper.wrapDegrees(Math.toDegrees(Math.atan2(e.getZ() - mc.player.getZ(), e.getX() - mc.player.getX())) - 90.0); return Math.abs(yaw - MathHelper.wrapDegrees(mc.player.getYaw())); }
