...
import thunder.hack.setting.impl.Bind; // Thêm import cho setbind
...
public class Aura extends Module {
    ...
    // KHÔI PHỤC ĐẦY ĐỦ CÁC CHẾ ĐỘ ESP GỐC
    ...
    // Giữ nguyên setting gốc nhưng có thể ghi đè bằng ElytraTarget mới
    public final Setting<Boolean> elytraTargetOld = new Setting<>("ElytraTarget(Old)", true);
    ...
    // Logic mới cho Sprint Mode
    private void handleSprintMode() {
        if (target == null) return;
        switch (sprintMode.getValue()) {
            case HvH -> {
                // Combo optimization: chỉ tắt sprint khi rơi để crit
                if (mc.player.fallDistance > 0.05) mc.player.setSprinting(false);
                else if (mc.player.forwardSpeed > 0) mc.player.setSprinting(true);
            }
            case Legit -> {
                if (mc.player.forwardSpeed <= 0 || mc.player.isHorizontalCollision || mc.player.isUsingItem()) 
                    mc.player.setSprinting(false);
            }
            case SMP -> {
                mc.player.setSprinting(mc.player.forwardSpeed > 0 && !mc.player.isUsingItem());
            }
            case None -> {}
        }
    }

    // Logic mới cho Elytra Target (Vượt đối thủ + Auto Firework)
    private void handleElytraTarget() {
        if (!elytraTargetGroup.getValue().isEnabled() || target == null || !mc.player.isFallFlying()) return;

        // Cố gắng giữ độ cao lớn hơn đối thủ để lấy lợi thế Crit không trung
        double desiredY = target.getY() + heightGain.getValue();
        if (mc.player.getY() < desiredY) {
            mc.player.addVelocity(0, 0.15, 0); // Nhích nhẹ lên trên
            
            // Auto Firework: Tự động dùng pháo hoa nếu có trong tay/hotbar
    ...
    // --- CÁC HÀM GỐC TỪ FILE BẠN GỬI (KHÔNG THAY ĐỔI) ---
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        // TOÀN BỘ ESP GỐC ĐƯỢC GIỮ LẠI Ở ĐÂY
        switch (esp.getValue()) {
    ...
    public Entity findTarget() {
        ...
        return switch (sort.getValue()) {
            ...
            case LowestDurability -> first_stage.stream().min(Comparator.comparing(e -> {
                float v = 0;
                for (ItemStack armor : e.getArmorItems()) if (armor != null && !armor.getItem().equals(Items.AIR)) v += ((armor.getMaxDamage() - armor.getDamage()) / (float) armor.getMaxDamage());
                return v;
            })).orElse(null);
