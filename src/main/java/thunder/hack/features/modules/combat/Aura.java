...
public class Aura extends Module {
    ...
    // THÊM: Elytra Target (Giữ kiểu Boolean để không lỗi compile các class khác)
    ...
    private void handleElytraTarget() {
        if (!elytraTarget.getValue() || target == null || !mc.player.isFallFlying()) return;
        double targetHeight = target.getY() + heightGain.getValue();
    ...
    public void auraLogic() {
        ...
        if (sprintMode.getValue() == SprintMode.HvH) {
        ...
        if (sprintMode.getValue() == SprintMode.Legit) {
            if (mc.player.forwardSpeed <= 0
        ...
        if (sprintMode.getValue() == SprintMode.SMP) {
