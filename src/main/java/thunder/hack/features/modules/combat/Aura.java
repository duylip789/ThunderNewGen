...
public class Aura extends Module {
        ...
        for (float x1 = -hb; x1 <= hb; x1 += 0.15f) {
            for (float z1 = -hb; z1 <= hb; z1 += 0.15f) {
                for (float y1 = 0.05f; y1 <= target.getBoundingBox().getLengthY(); y1 += 0.25f) {
                    Vec3d pos = new Vec3d(target.getX() + x1, target.getY() + y1, target.getZ() + z1);
                    if (PlayerUtility.squaredDistanceFromEyes(pos) > getSquaredRotateDistance()) continue;
                    float[] rotation = Managers.PLAYER.calcAngle(pos);
    ...
    public Entity findTarget() {
        ...
        return switch (sort.getValue()) {
            ...
            case LowestDurability -> first_stage.stream().min(Comparator.comparing(this::getArmorDurability)).orElse(null);
            case HighestDurability -> first_stage.stream().max(Comparator.comparing(this::getArmorDurability)).orElse(null);
        };
    }
    ...
    @Override
    public void onDisable() { target = null; ghostList.clear(); }
